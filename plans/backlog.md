...
- [ ] Before deploying the final unique-index change, check a separate copy of production data for case/space-insensitive duplicate item names, take a consistent backup, and verify startup against the copy. This change is deliberately above the deployable pre-migration revision; do not include it in today's deploy.
