...
- [ ] Complete [read-only offline viewing's outstanding HTTPS and real-device checks](offline-readonly.md#verification-plan-and-acceptance-gates). The snapshot API, one-room IndexedDB save, and generic service-worker fallback are implemented and tested in desktop HTTP Chromium/Firefox; actual installed iPhone/Android launch and persistence are not yet verified. See [current behavior](../docs/home-screen-installation.md#read-only-offline-viewing-on-a-prepared-browser).
