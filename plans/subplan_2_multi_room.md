...
- [x] Phase 2: Add room functions to `database_crud.py` and update list functions to return `room_slug`.
- [x] Phase 3: Build the Lobby UI at `/` to show/create rooms and handle room unlocking.
- [x] Phase 4: Build the Room Dashboard at `/room/{slug}` with secure session checks, Rename/Delete controls, and update the List View's "Back" button.